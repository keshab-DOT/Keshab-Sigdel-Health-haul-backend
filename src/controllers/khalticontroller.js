...
export const initiateKhaltiPayment = async (req, res) => {
  try {
    ...
    if (!process.env.KHALTI_SECRET_KEY) {
      return res.status(500).json({ message: "Khalti secret key not configured in .env" });
    }
    ...
    const productDetails = order.products.map(item => ({
      identity: item.productId?._id?.toString() || item.productId?.toString() || "unknown",
    ...
    const payload = {
      ...
      amount_breakdown: [{ label: "Medicines", amount: amountPaisa }],
    ...
    return res.status(500).json({
      message: err?.response?.data?.detail || err?.response?.data?.message || err.message || "Failed to initiate Khalti payment",
    });
  }
};
...
export const verifyKhaltiPayment = async (req, res) => {
  try {
    ...
    if (khaltiData.status === "Completed") {
      ...
      await order.save();
      return res.json({ status: "Completed", order, transactionId: khaltiData.transaction_id });
    }
